...
// Load astrological interpretation data
function loadAstrologicalData() {
  try {
    const dataPath = path.join(process.cwd(), 'src', 'data', 'astrological-interpretations.json')
...
export async function POST(req) {
  ...
  const messages = [
    ...
    {
      role: 'user',
      content: `Desde el punto de vista astrológico, basado en astrología evolutiva, interpreta, como si fueras un astrólogo amigo (no en forma de lista de aspectos sino como una conversación más íntima y profunda), los siguientes aspectos, como si fuera un mini-regalito de intepretación para la persona para generarle interés en indagar todavía más en su carta (nota que no sabremos el sexo de la persona así que manten las respuesta genérica sin asumir sexo hombre o mujer). Enfócate solamente en la información del Sol y los Nodos Lunares.Necesito que regreses esa interpretación de la siguiente manera: En formato json { short: 'enunciado resumiendo la esencia de la persona', mission: 'Estás aquí para...', long: '4-5 párrafos enfocados en un mensaje positivo para la persona' }:\n\n${formattedPlanets.join('\n')}`
      // content: `Desde el punto de vista astrológico, basado en astrología evolutiva, interpreta, como si fueras un astrólogo amigo (no en forma de lista de aspectos sino como una conversación más íntima y profunda), los siguientes aspectos, como si fuera un mini-regalito de intepretación para la persona para generarle interés en indagar todavía más en su carta (nota que no sabremos el sexo de la persona así que manten las respuesta genérica sin asumir sexo hombre o mujer). Necesito que regreses esa interpretación de la siguiente manera: En formato json { short: 'enunciado resumiendo la esencia de la persona', mission: 'Estás aquí para...', long: '4-5 párrafos enfocados en un mensaje positivo para la persona' }:\n\n${formattedPlanets.join('\n')}`
  ...
  try {
    ...
    const rawInterpretation = completion?.choices?.[0]?.message?.content
    ...
    const interpretation = rawInterpretation.replace(/^```json\s*/, '').replace(/```$/, '')
